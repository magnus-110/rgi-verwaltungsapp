...
async function setPasswordWithRetry(admin: any, userId: string): Promise<{ password: string; error: any | null }> {
  for (let i = 0; i < 3; i++) {
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const shouldResetExistingPassword = typeof force_reset_password === "boolean" ? force_reset_password : !!send_email;
    ...
    if (authUserId) {
      ...
      if (profileMatch?.user_id) {
        ...
        while (!existingUser) {
          ...
          const found = (pageData.users as any[]).find(
      ...
      if (existingUser) {
        ...
        if (!createdUserId) {
          // E-Mail ist meist schon vergeben ("email already registered").
          // Dann vorhandenes Konto suchen und verknüpfen statt abzubrechen.
          ...
          if (foundId) {
            ...
            if (shouldResetExistingPassword) {
              ...
              if (pwErr) {
                return new Response(
                  JSON.stringify({ error: `Passwort konnte nicht gesetzt werden: ${pwErr.message}` }),
                  {
                    status: 500,
                    headers: { ...corsHeaders, "Content-Type": "application/json" },
                  },
            ...
            return new Response(
              JSON.stringify({
                error: `Diese E-Mail (${email}) ist im Anmeldesystem bereits belegt, ein zugehöriges Konto wurde aber nicht gefunden (evtl. verwaiste Identität). Bitte Support kontaktieren.`,
              }),
