-- Improve invoice fallback matching (vendor + amount) for Make.com bookings
...
BEGIN
  BEGIN
    ...
    -- 1) Invoice via receipt_number == invoice_number (normalized)
    ...
    -- 2) Fallback invoice via vendor_name in description + gross amount
    IF NEW.invoice_id IS NULL AND matched_inv.id IS NULL AND NEW.description IS NOT NULL THEN
      SELECT i.id, i.vat_amount, i.gross_amount, i.net_amount
      INTO matched_inv
      FROM public.invoices i
    ...
    IF NEW.invoice_id IS NULL AND matched_inv.id IS NOT NULL THEN
    ...
    -- 3) Template fallback if still no invoice
...
-- Re-run matching for recent KI/Make rows still missing mapping
