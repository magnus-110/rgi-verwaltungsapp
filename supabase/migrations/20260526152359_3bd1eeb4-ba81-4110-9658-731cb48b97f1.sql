...
BEGIN
  ...
  BEGIN
    ...
    FROM (
      ...
      FROM weg_reports r JOIN buildings b ON b.id = r.building_id
      WHERE r.building_id = ANY(v_building_ids) AND r.created_at > now() - interval '7 days' AND p_management_mode = 'weg'
      UNION ALL
      SELECT jsonb_build_object('type','report','id',r.id,'title',r.title,'building_name',b.name,'occurred_at',r.created_at)
      FROM miete_reports r JOIN buildings b ON b.id = r.building_id
      WHERE r.building_id = ANY(v_building_ids) AND r.created_at > now() - interval '7 days' AND p_management_mode = 'rent'
      UNION ALL
  ...
  BEGIN
    SELECT coalesce(jsonb_agg(jsonb_build_object(
      ...
      'open_reports', coalesce((SELECT count(*) FROM weg_reports WHERE building_id = b.id AND status='open' AND p_management_mode='weg'),0)
        + coalesce((SELECT count(*) FROM miete_reports WHERE building_id = b.id AND status='open' AND p_management_mode='rent'),0),
