...
ALTER TABLE public.billing_templates
  ...
  CHECK (scope = ANY (ARRAY['overall'::text, 'single'::text, 'asset_report'::text, 'paragraph_35a'::text, 'economic_plan'::text]));

DROP TABLE IF EXISTS public.paragraph_35a_templates CASCADE;