DROP FUNCTION IF EXISTS public.get_proxy_meeting_state(uuid);

CREATE OR REPLACE FUNCTION public.get_proxy_meeting_state(p_token text)
RETURNS jsonb
LANGUAGE plpgsql
STABLE
SECURITY DEFINER
...
GRANT EXECUTE ON FUNCTION public.get_proxy_meeting_state(text) TO anon, authenticated;