...
export function CreateContactDialog({ open, onOpenChange, onCreated }: Props) {
  ...
  const handleSave = async () => {
    ...
    // Insert phones, emails, banks in parallel
    ...
    await Promise.all([
      validPhones.length > 0
        ? supabase.from("contact_phones").insert(validPhones.map((p, i) => ({ contact_id: contact.id, phone_number: p.phone_number, label: p.label })))
        : Promise.resolve(),
      validEmails.length > 0
        ? supabase.from("contact_emails").insert(validEmails.map((e, i) => ({ contact_id: contact.id, email: e.email, label: e.label, is_primary: i === 0 })))
        : Promise.resolve(),
      validBanks.length > 0
            ...
            {phones.map((p, i) => (
              <div key={i} className="flex items-center gap-2">
                ...
                <Input
                  ...
                  onChange={(e) => { const u = [...phones]; u[i].phone_number = e.target.value; setPhones(u); }}
            ...
            {emails.map((e, i) => (
              <div key={i} className="flex items-center gap-2">
                ...
                <Input
                  type="email"
                  value={e.email}
                  onChange={(ev) => { const u = [...emails]; u[i].email = ev.target.value; setEmails(u); }}
              ...
              <div key={i} className="bg-muted/40 rounded-lg p-3 space-y-2">
                ...
                <div>
                  ...
                  <Input
                    ...
                    onChange={(e) => { const u = [...banks]; u[i].iban = e.target.value.replace(/\s/g, ''); setBanks(u); }}
                ...
                <div className="grid grid-cols-2 gap-2">
                  <div>
                    ...
                    <Input
                      value={b.bic}
                      onChange={(e) => { const u = [...banks]; u[i].bic = e.target.value; setBanks(u); }}
                      className="font-mono text-sm"
                    />
                  </div>
                  <div>
                    ...
                    <Input
                      value={b.bank_name}
                      onChange={(e) => { const u = [...banks]; u[i].bank_name = e.target.value; setBanks(u); }}
                    />
                  </div>
                </div>
                <div>
                  ...
                  <Input
                    ...
                    onChange={(e) => { const u = [...banks]; u[i].account_holder = e.target.value; setBanks(u); }}
                  />
                </div>
              </div>
            ))}
