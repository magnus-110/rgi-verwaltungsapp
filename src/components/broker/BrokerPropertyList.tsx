  ...
  const { data: items = [] } = useQuery({
    ...
    queryFn: async () => {
      let q = supabase.from('broker_properties' as any)
        .select('id, title, address, city, is_active')
...
const CreateBrokerPropertyDialog = ({ open, onOpenChange, listingType, onCreated }: {
  open: boolean;
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="sm:max-w-md">
        <DialogHeader>
          <DialogTitle>Neues {listingType === 'rent' ? 'Vermietungs-' : 'Verkaufs-'}Objekt</DialogTitle>
        </DialogHeader>
        <div className="space-y-3">
          ...
          <div>
            <Label>Adresse</Label>
            <Input value={address} onChange={(e) => setAddress(e.target.value)} placeholder="Optional, später im Detail" />
          </div>
        </div>
        <DialogFooter>
          <Button variant="outline" onClick={() => onOpenChange(false)}>Abbrechen</Button>
          <Button onClick={submit} disabled={loading || !title.trim()}>Anlegen</Button>
        </DialogFooter>
      </DialogContent>
    </Dialog>
  );
