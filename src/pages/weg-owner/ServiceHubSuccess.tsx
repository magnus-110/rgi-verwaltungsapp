...
export function WegOwnerServiceHubSuccess() {
  ...
  const handleDownload = async (index?: number) => {
    ...
    try {
      const { data, error } = await supabase.functions.invoke("get-service-document-url", {
  ...
  return (
    <div className="max-w-2xl mx-auto px-4 py-10">
      <Button variant="ghost" size="sm" onClick={() => navigate("/weg-owner/service-hub")} className="mb-4">
      ...
      <Card className="p-8 text-center space-y-4">
        {!order ? (
          ...
          <>
            ...
            <h1 className="text-2xl font-bold" style={{ fontFamily: "Century Gothic, sans-serif" }}>
              Vielen Dank für Ihre Bestellung
            </h1>
            <div className="flex justify-center gap-2">
              <Badge variant={paid ? "default" : "secondary"}>{paid ? "Bezahlt" : order.status}</Badge>
            ...
            {ready ? (
              <>
                <p className="text-muted-foreground text-sm">
                  {docs.length > 1 ? "Ihre Dokumente sind fertig." : "Ihr Dokument ist fertig."}
                </p>
              ...
              <div className="space-y-3">
                <p className="text-sm text-amber-700">Bei der Erstellung Ihres Dokuments ist ein Fehler aufgetreten.</p>
              ...
              <div className="space-y-3">
                <p className="text-muted-foreground text-sm flex items-center justify-center gap-2">
                  ...
                  Wir erstellen Ihr Dokument. Das kann einen Moment dauern (in der Regel unter einer Minute). Sie können
                  diese Seite offen lassen – das Dokument wird zusätzlich per E-Mail versendet.
                </p>
                {waitedTooLong && (
              ...
              <p className="text-muted-foreground text-sm">Sobald die Zahlung bestätigt ist, geht es hier weiter.</p>
            )}
